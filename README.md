...
An OpenClaw Skill that automates full agent migration. Your agent packs everything on the old device, you deploy with a single command on the new device, and your agent is back online — configs, memory, skills, credentials, all intact.
...
> 🐧 **Linux → Linux migration only.** Both old and new devices must be Linux (Ubuntu recommended).

**New device:**
- Ubuntu 22.04 / 24.04 (other Debian-based distros may work)
- 2-core CPU, 2GB+ RAM
- SSH access with sudo

**Old device:**
- Running OpenClaw installation (any Linux)
...
> 🐧 **仅支持 Linux → Linux 迁移。** 新旧设备均需为 Linux 系统（推荐 Ubuntu）。

**新设备：**
- Ubuntu 22.04 / 24.04（其他 Debian 系发行版可能兼容）
- 2 核 CPU，2GB+ 内存
- SSH 登录 + sudo 权限

**旧设备：**
- 已运行 OpenClaw 的 Linux 设备
